...
export async function GET(request: NextRequest) {
  ...
  if (!apiKey || apiKey === 'your_api_key_here') {
    return NextResponse.json(
      { error: 'OpenWeather API key not configured. Please add your key to .env.local' },
      { status: 500 }
    );
  }
  ...
  if (lat && lon) {
    ...
    url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(q)}&units=${units}&appid=${apiKey}`;
  } else {
